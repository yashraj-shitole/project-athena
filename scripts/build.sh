...
#   ./scripts/build.sh --clean               remove existing athena-* images first
#   ./scripts/build.sh --no-cache            ignore the layer cache
#   ./scripts/build.sh --service backend    build one service (backend|frontend|web-dev|db|redis|ollama|ai-service|worker)
#   ./scripts/build.sh --include-dev        also build the web-dev (Vite) image
#   ./scripts/build.sh --production          enforce prod-grade env validation
#   ./scripts/build.sh --verbose             --progress=plain
set -euo pipefail
...
log_info 'Next: ./scripts/docker-up.sh            (run the stack)'
log_info '     ./scripts/docker-up.sh --debug     (hot reload + debugger)'