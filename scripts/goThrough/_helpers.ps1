...
function Ensure-NetworksAndVolumes {
    ...
    # The project default network must carry the
    # com.docker.compose.network=default label — otherwise `docker compose up`
    # refuses to reuse it ("a network with name athena_default exists but was
    # not created by compose"). Earlier versions of this script created the
    # network with no labels, so a stale, unlabeled one may already be sitting
    # around; inspect first, and if it's empty + unlabeled, remove and
    # recreate it with the right label. Errors are swallowed (best-effort).
    try {
        $existing = & docker network inspect $network --format '{{json .Labels}}' 2>$null
        if ($LASTEXITCODE -ne 0) {
            $null = & docker network create --label com.docker.compose.network=default --label com.docker.compose.project=$script:PROJECT_NAME $network 2>$null
        } else {
            $hasLabel = $false
            if ($existing -and $existing -ne 'null') {
                try { $hasLabel = ($existing | ConvertFrom-Json).'com.docker.compose.network' -eq 'default' } catch { $hasLabel = $false }
            }
            if (-not $hasLabel) {
                # Empty + unlabeled → safe to drop and recreate with the label.
                $containerCount = (& docker network inspect $network --format '{{len .Containers}}' 2>$null)
                if ($containerCount -eq '0' -or [string]::IsNullOrEmpty($containerCount)) {
                    $null = & docker network rm $network 2>$null
                    $null = & docker network create --label com.docker.compose.network=default --label com.docker.compose.project=$script:PROJECT_NAME $network 2>$null
                    Write-Info "recreated network $network with compose labels"
                } else {
                    Write-Warn "Network $network is in use but lacks the compose label; leaving alone (manual fix needed)."
                }
            }
        }
    } catch {}
