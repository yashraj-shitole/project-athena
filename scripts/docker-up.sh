...
#
# Usage:
#   ./scripts/docker-up.sh                       normal mode
#   ./scripts/docker-up.sh --debug               debug mode (hot reload + debugger)
#   ./scripts/docker-up.sh --debug --watch        watch mode (follow logs)
#   ./scripts/docker-up.sh --services backend,frontend
#   ./scripts/docker-up.sh --build               rebuild before starting
set -euo pipefail
...
if [[ $WATCH -eq 1 ]]; then
  ...
  log_info 'Containers are still running. Stop with: ./scripts/goThrough/docker-down.sh'
fi