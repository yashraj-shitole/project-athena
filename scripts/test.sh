...
#
# Usage:
#   ./scripts/test.sh                 run all (backend unit + frontend + e2E[skip])
#   ./scripts/test.sh --backend        backend unit tests only
#   ./scripts/test.sh --frontend       frontend tests only (skipped if none configured)
#   ./scripts/test.sh --integration     backend integration tests (starts infra)
#   ./scripts/test.sh --e2e             E2E (skipped  -  Phase 2)
#   ./scripts/test.sh --coverage        backend unit tests with pytest-cov
#   ./scripts/test.sh --ci              CI mode: unit + coverage, fail-fast
#   ./scripts/test.sh --verbose         verbose pytest
set -euo pipefail
