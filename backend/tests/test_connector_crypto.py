...
def test_explicit_key_takes_precedence(monkeypatch):
    ...
    monkeypatch.setenv("ATHENA_JWT_SECRET", "completely-different-secret")
    crypto.reset_cache()
    config_module.get_settings.cache_clear()  # type: ignore[attr-defined]
    assert crypto.decrypt(blob) == plain
...
def test_dev_fallback_with_hkdf(monkeypatch):
    ...
    monkeypatch.setenv("ATHENA_JWT_SECRET", "stable-dev-secret")
    ...
    monkeypatch.setenv("ATHENA_JWT_SECRET", "rotated-dev-secret")
    crypto.reset_cache()
    config_module.get_settings.cache_clear()  # type: ignore[attr-defined]
    with pytest.raises(crypto.CryptoError):
        crypto.decrypt(blob)
...
def test_prod_without_explicit_key_raises(monkeypatch):
    ...
    monkeypatch.setenv("ATHENA_JWT_SECRET", "real-jwt-secret")
